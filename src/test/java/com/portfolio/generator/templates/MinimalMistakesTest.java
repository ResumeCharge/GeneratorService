...
class MinimalMistakesTest {
  ...
  void testProcessTemplate()
    ...
    extraDetails.put(
        "resumeLink", "https://test-bucket-pgen.s3.us-west-2.amazonaws.com/1234.resume.pdf");
    resume.setExtraDetails(extraDetails);

    resume.setUUID(uuid);
    final WebsiteDetailsModel websiteDetails = new WebsiteDetailsModel();
    websiteDetails.setTemplateName("Minimal Mistakes");
    ...
    verify(deploymentStatusHelperMock, times(4))
        .updateDeploymentProgress(any());
    verify(actionProcessorMock, times(13))
        .processAction(any(), any());
    verify(templateProcessorMock, times(5))
        .processTemplate(any(), any(), any(), any());
  }
}
