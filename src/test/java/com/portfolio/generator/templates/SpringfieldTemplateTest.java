...
class SpringfieldTemplateTest {
  ...
  void testProcessTemplate()
    ...
    websiteDetails.setTemplateName("springfield");
    ...
    verify(gitHubServiceMock, times(1)).deployNewGitHubPagesWebsite(any(PortfolioGenerationTask.class));
    verify(deploymentStatusHelperMock, times(4)).updateDeploymentProgress(any());
    verify(actionProcessorMock, times(11)).processAction(any(), any());
    verify(templateProcessorMock, times(8)).processTemplate(any(), any(), any(), any());
  }
}
